...
import { Badge, CalendarCheck2, CheckCircle2, Inbox, Layers3, LinkIcon, MousePointerClick, Share2, ShieldCheck, Sparkles, UploadCloud } from "lucide-react";
...
const reveal = { hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0, transition: { duration: rm ? 0 : 0.7, ease: [0.22, 1, 0.36, 1] } } };
const groupStagger = { hidden: {}, show: { transition: { staggerChildren: 0.08, delayChildren: 0.06 } } };
     ...
     <div className="min-h-screen text-slate-100 [--bg1:#0b1220] [--bg2:#030711]">
      ...
      <CursorGlow/>
       ...
       {/* hero section */}
        ...
        <div className="mx-auto max-w-7xl px-6 pt-24 pb-16 sm:pt-28 lg:pt-32">
          <motion.div variants={groupStagger} initial="hidden" animate="show" className="grid items-center gap-12  sm:grid-cols-1 lg:grid-cols-2">
            <motion.div variants={reveal}>
              ...
              <motion.p variants={reveal} className="mt-6 max-w-xl text-lg text-slate-200/90">
                Drag fields into place, share your form, and watch responses roll in — all in one simple workspace.
              </motion.p>
              <motion.div variants={reveal} className="mt-8 flex flex-wrap items-center gap-3">
                <Button size="lg" className="rounded-2xl">Start building</Button>
                <Button size="lg" variant="outline" className="rounded-2xl">See a live demo</Button>
              ...
              <motion.div variants={reveal} className="mt-10 flex flex-wrap items-center gap-6">
                ...
                <div className="flex items-center gap-4 text-slate-300">
                  <ShieldCheck className="h-4 w-4" /> <span className="text-xs">Private by default</span><span>•</span>
                  <CalendarCheck2 className="h-4 w-4" /> <span className="text-xs">Quick to launch</span>
                </div>
              </motion.div>
            </motion.div>
            <motion.div variants={reveal}><ParallaxPreview /></motion.div>
          </motion.div>
        </div>
      </section>
      ...
      <section id="features" className="mx-auto max-w-7xl px-6 pb-10">
        <div className="mb-8 flex items-end justify-between">
          <h2 className="text-2xl font-semibold sm:text-3xl">Everything you need to collect answers</h2>
          <div className="text-sm text-slate-300">Fast · friendly · flexible</div>
        </div>
      </section>

      <div className="space-y-6 px-0">
        {FEATURES_BIG.map((f, index) => (
          <div key={f.title} data-aos={index % 2 === 0 ? "fade-right" : "fade-left"} className={`relative isolate overflow-hidden py-12 sm:py-16`}>
            <div className={`absolute inset-0 -z-10 bg-gradient-to-br ${f.accent}`} />
            <div className="mx-auto flex max-w-7xl items-center gap-8 px-6">
              <div className="shrink-0">
                <div className="grid h-14 w-14 place-items-center rounded-2xl bg-slate-900/60 ring-1 ring-inset ring-slate-700">
                  {React.createElement(f.icon, { className: "h-7 w-7 text-emerald-300" })}
                </div>
              </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12" data-aos="fade-up">
        <div className="mb-4 flex items-center gap-2"><Badge className="bg-slate-800 text-slate-300">Highlights</Badge><span className="text-slate-400 text-sm">Swipe →</span></div>
        <div className="no-scrollbar flex snap-x gap-3 overflow-x-auto rounded-2xl border border-slate-800/70 bg-slate-950/40 p-3">
          {MINI_FEATURES.map((mf) => (
            <div key={mf.label} className="snap-start shrink-0 rounded-xl border border-slate-800 bg-slate-900/70 px-4 py-3 text-sm text-slate-200 inline-flex items-center gap-2">
              {React.createElement(mf.icon, { className: "h-4 w-4 text-emerald-300" })}
              {mf.label}
            </div>
          ))}
        </div>
       ...
       {/* TIMELINE */}
      <section id="how" className="mx-auto max-w-7xl px-6 py-12" data-aos="fade-up">
        <div className="mb-6 flex items-center gap-3">
          <Badge className="bg-cyan-500/15 text-cyan-300 ring-1 ring-inset ring-cyan-400/30">How it works</Badge>
          <span className="text-slate-300">Design → Share → Collect</span>
        </div>
        <ol className="relative ml-2 border-l border-slate-800 pl-6">
          {STEPS.map((s, idx) => (
            <li key={s.title} className="mb-10" data-aos="fade-up" data-aos-delay={idx * 80}>
      ...
      <section className="mx-auto max-w-7xl px-6" id="end-to-end">
        <div className="mb-6 flex items-center gap-2">
          <Badge className="bg-emerald-500/15 text-emerald-300 ring-1 ring-inset ring-emerald-400/30">End‑to‑end flow</Badge>
          <span className="text-slate-300">All the way from idea to answers</span>
        </div>

        <div className="grid grid-cols-1 gap-4 sm:grid-cols-6 lg:grid-cols-12">
          {/* Tall Steps (left) */}
          <BentoCard title="Steps" subtitle="From idea to publish" icon={CalendarCheck2} className="sm:col-span-6 lg:col-span-4 lg:row-span-2">
            <ol className="mt-4 space-y-3">
              {[
                ...
                <li key={x.t} className="flex items-center gap-2 rounded-xl border border-slate-800/70 bg-slate-950/40 px-3 py-2 text-sm text-slate-200">
                  {React.createElement(x.i, { className: "h-4 w-4 text-emerald-300" })}
                  <span className="truncate">{x.t}</span>
                  <span className="ml-auto rounded-full border border-slate-800/60 bg-slate-900/60 px-2 py-0.5 text-[10px] text-slate-400">{idx + 1}</span>
                </li>
              ))}
            </ol>
          </BentoCard>
          ...
          {/* Big Visual (center) */}
          <BentoCard title="Design visually" subtitle="Live preview • Drag to reorder" icon={Layers3} className="sm:col-span-6 lg:col-span-5">
            <div className="relative mt-5 rounded-2xl border border-slate-800/70 bg-slate-950/40 p-4">
              <div className="grid grid-cols-3 gap-3">
                {["Text", "Email", "Select", "Date", "File", "Radio"].map((f) => (
                  <div key={f} className="rounded-xl border border-slate-800/80 bg-slate-900/70 px-3 py-2 text-center text-xs text-slate-200">{f}</div>
                ))}
              </div>
          ...
          {/* Save (top-right) */}
          <BentoCard title="Auto‑save" subtitle="Your work is safe" icon={UploadCloud} className="sm:col-span-3 lg:col-span-3">
            <div className="mt-4 flex items-center gap-2 rounded-xl border border-slate-800/70 bg-slate-950/40 p-3 text-sm text-slate-300">
              <CheckCircle2 className="h-4 w-4 text-emerald-300" /> Saved a few seconds ago
            </div>
          </BentoCard>

          {/* Share (bottom-right) */}
          <BentoCard title="Share with a link" subtitle="/f/team-survey" icon={LinkIcon} className="sm:col-span-3 lg:col-span-3">
            <div className="mt-4 rounded-xl border border-slate-800/70 bg-slate-950/40 p-4">
              <div className="text-sm text-slate-300">One click publish</div>
              <div className="mt-2 text-xs text-slate-400">Embed anywhere or copy the URL.</div>
            </div>
          </BentoCard>
        </div>
      </section>
       ...
       {/* Testimonials */}
      <section className="mx-auto max-w-7xl px-6 py-12" data-aos="fade-up">
        <div className="mb-4 flex items-center gap-2"><Badge className="bg-slate-800 text-slate-300">Loved by teams</Badge><Sparkles className="h-4 w-4 text-emerald-300" /></div>
        <div className="rounded-2xl border border-slate-800/70 bg-gradient-to-br from-slate-900 to-slate-950 p-6">
          <div className="grid gap-6 md:grid-cols-3">
            {["We replaced our old form tool in a day.", "The share link made sign‑ups so easy.", "Exporting results saved us hours."].map((q) => (
              <blockquote key={q} className="text-slate-200">“{q}”</blockquote>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="overflow-hidden rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-slate-900 to-slate-950 p-8 sm:p-10">
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
            ...
            <div className="flex gap-3">
              <Button size="lg" className="rounded-2xl">Start building</Button>
              <Button size="lg" variant="secondary" className="rounded-2xl">View a sample form</Button>
            </div>
          </div>
        </div>
      </section>
